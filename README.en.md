...
| [DSH-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar) | 3545 | 🟢 ok | Sidebar workbench: file render/terminal/git/subagent |
...
| [dsh-pet](https://github.com/PC2005-cloud/dsh-pet) | 609 | 🟢 ok | DeepSeek Harness 桌面宠物插件 + 完整素材生成链：AI 提示词 → 绿幕视频 → 透明动画 → 可安装插件，从零到宠物全流程可复现 |
...
| [dsh-redteam-model](https://github.com/SeaOf0/dsh-redteam-model) | 408 | 🟢 ok | 基于dsh web实现的多种模式，目的是服务于redteam进行授权的安全研究，覆盖渗透测试、红队评估、代码审计等范围领域，请勿用于非法行为。（允许二开，赋予模块各位自己的业务逻辑，不定期完善逻辑与修复bug） |
...
| [dsh-infinite-gen-2](https://github.com/Minglink/dsh-infinite-gen-2) | 1371 | 🔴 broken (dump-config exit=0 but plugin not found in composed tree) | DeepSeek 专用破甲插件「无限二代」dsh-infinite-gen-2 — armor-breaking plugin for DeepSeek稳定化破甲提示词，求 Star 收藏 ⭐ |
...
| [dsh-infinite-gen-3](https://github.com/Minglink/dsh-infinite-gen-3) | 1371 | 🟢 ok | DeepSeek 专用破甲插件「无限三代」dsh-infinite-gen-3 — armor-breaking plugin for DeepSeek，破甲版：稳定化破甲，求 Star 收藏 ⭐ |
...
| [dsh-industry-research](https://github.com/PerryLink/dsh-industry-research) | 95 | 🟢 ok | Industry and company research domain pack for DeepSeek Harness: methodology skills, industry chain mapping, public-source policy/… |
...
| [dsh-im](https://github.com/xmanrui/dsh-im) | 1278 | 🟢 ok | 通过扫码把IM机器人接入DeepSeek Harness（支持飞书、微信、钉钉、QQ、企业微信等） |
...
| [DeepSeek-Balance-Whale-Widget](https://github.com/MeteorNOX/DeepSeek-Balance-Whale-Widget) | 2248 | 🔴 broken (dump-config exit=0 but plugin not found in composed tree) | DeepSeek Harness（DSH）一只住在 DSH 界面右下角的小鲸鱼娘，帮你盯着DeepSeek账户余额。QQ弹弹，支持拖拽吸附、左吸附翻转、数字滚动动画，随界面自动启用，建议直接喊来你的dsh安装 |
...
| [dsh-infinite-gen-4](https://github.com/Minglink/dsh-infinite-gen-4) | 1371 | ⚪ unknown | DeepSeek v4.1 flash 网络安全红队工具（无限四代） — jailbreak prompts and test suite for DeepSeek 求 Star 收藏 ⭐ |
...
| [billion-context-dsh](https://github.com/Tyan66666/billion-context-dsh) | 86 | 🟢 ok | Model-driven context management (Active Context Pruning / ACP) for the DeepSeek Harness — the model decides when and what to comp… |
...
| [dsh-auto-review](https://github.com/PerryLink/dsh-auto-review) | 161 | 🟢 ok | Second-model AI auto-review for DeepSeek Harness approval requests: a read-only reviewer subagent returns structured allow/deny v… |
...
| [dsh-context](https://github.com/bowenliang123/dsh-context) | 1350 | 🟢 ok | A DeepSeek Harness plugin for Context insight panel — a Context tab in the web UI showing what the model's context window is made… |
...
| [MemOS](https://github.com/MemTensor/MemOS) | 11295 | ⚪ unknown | Self-evolving memory OS for LLM & AI Agents: ultra-persistent memory, hybrid-retrieval, and cross-task skill reuse, with 35.24% t… |
...
| [dsh-config-manager](https://github.com/xiajiajun516/dsh-config-manager) | 110 | 🟢 ok | Backup / export / import / migrate your DeepSeek Harness (DSH) configuration - dual-face Cordis plugin (host engine + Web UI). On… |
...
| [dsh-nexttavern](https://github.com/a86582751/dsh-nexttavern) | 12 | ⚪ unknown | 原生 Agent 驱动的长篇角色扮演：交互式创作、主动世界书、动态记忆、多角色推演与小说导出。创作一个世界，走进它，再把它带走。 |
...
| [deepseek-harness-studio](https://github.com/fufankeji/deepseek-harness-studio) | 632 | ⚪ unknown | DeepSeek Harness Studio：面向 DeepSeek Harness 的现代化桌面开发环境，内置插件中心、视觉增强与本地 Host。 |
...
| [dsh-purge](https://github.com/YuJunZhiXue/dsh-purge) | 694 | 🟢 ok | DeepSeek Harness 破甲：让所有模型都能破甲，不同模型可换不同提示词；默认提示词面向国模「小码酱」。Jailbreak for every model — swap prompts per model. 求 Star 收藏 ⭐ |
...
| [ccteam](https://github.com/firstintent/ccteam) | 521 | ⚪ unknown | Orchestrates Claude Code/Codex/Grok/Kimi into one team |
...
| [dsh-Mimir-Academic-research](https://github.com/1692775560/dsh-Mimir-Academic-research) | 334 | ⚪ unknown | Mimir — 一站式科研工作台插件：LaTeX 论文边写边编译、arXiv 文献管理、实验追踪、指标图表、GPU 服务器 SSH 任务编排，管理科研全周期。An open-source research workbench plugin for the w… |
...
| [loopx](https://github.com/huangruiteng/loopx) | 5814 | ⚪ unknown | Long-horizon agent control plane for durable, governed work across Codex, Claude Code, and other harnesses. |
...
| [dsh-hub-cli](https://github.com/pax-beehive/dsh-hub-cli) | 286 | ⚪ unknown | Open-source CLI, schemas, resolver, and DSH agent tools for DSH Plugin Hub |
...
| [dsh-pocket](https://github.com/shaobeichen/dsh-pocket) | 1092 | 🟢 ok | 把 DeepSeek Harness 装进你的口袋：电脑上跑 dsh web，手机扫码即同步访问（局域网 + 公网，实时同屏） |
...
| Plugin | ⭐ | Compat | Description |
|---|---|---|---|
| [EchoBird](https://github.com/edison7009/EchoBird) | 3219 | ⚪ unknown | One-click install + model switch across 20+ coding agents |
