    ...
    function StoreTab(props) {
          ...
          SCENES.map((sc) => {
            ...
            return h('div', { key: sc.id, style: { ...C.card, marginBottom: '12px', minHeight: 0 } },
              ...
              sc.items.map((it) => {
                ...
                return h('div', { key: k, style: { display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '6px 0', borderTop: '1px dashed var(--dsw-alias-bg-layer-2)' } },
      ...
      const cards = filtered.slice(0, visible).map((p) => { // v0.7 F-G: 只渲染前 visible 条
        ...
        return h('div', { key: p.id || p.name, style: C.card, onClick: () => setDetail(p), title: t('dtOpen') },
          ...
          res && res.ok === true && res.mounted === false ? h('div', { style: { color: 'var(--dsw-alias-state-warn-primary)', fontSize: '11px', padding: '6px', border: '1px solid #9e6a03', borderRadius: '6px' } }, '⚠ ' + t('installedNotMounted') + ': ' + (res.installed || []).join(', ')) : null,
