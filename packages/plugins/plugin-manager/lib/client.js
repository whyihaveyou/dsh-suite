...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      onlyOk: '只看 🟢 兼容',
      dtMeta: '基本信息', dtEnv: '实测环境', dtDesc: '描述', dtCompat: '兼容性', dtRisk: '风险标注（证据而非背书）',
    ...
    const en = {
      ...
      onlyOk: '🟢 ok only',
      dtMeta: 'Info', dtEnv: 'Tested env', dtDesc: 'Description', dtCompat: 'Compatibility', dtRisk: 'Risk flags (evidence, not endorsement)',
      ...
      if (detail) {
        ...
        drawerEl = h('div', { style: C.drawerOverlay, onClick: () => setDetail(null) },
          h('div', { style: C.drawer, onClick: (e) => e.stopPropagation() },
            ...
            d.compatDshVersion ? h('div', { style: { fontSize: '11px', color: 'var(--dsw-alias-label-secondary)', marginTop: '6px' } }, '🔬 ' + t('dtEnv') + ': DSH v' + d.compatDshVersion + (d.compatLastVerified ? ' @ ' + d.compatLastVerified : '')) : null,
            h('div', null,
