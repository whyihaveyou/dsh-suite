#!/usr/bin/env python3
"""Generate dsh-suite X(Twitter) brand assets: avatar.png + banner.png (DeepSeek-blue theme, banner right-shifted to clear the profile avatar)."""
...
BG       = (11, 15, 26)
BG_SOFT  = (15, 21, 38)
BRAND    = (77, 107, 254)   # DeepSeek blue #4d6bfe
BRAND_DK = (18, 27, 78)     # dark navy (text on brand tile)
TEXT     = (230, 234, 242)
MUTED    = (148, 163, 184)
...
def make_avatar():
    ...
    cx = W // 2
    tile_top = int(H * 0.30)
    rounded_rect(d, [cx - tile//2, tile_top, cx + tile//2, tile_top + tile], 44*S, BRAND)
    ...
    d.text((cx - dsw/2 - dsh_bbox[0], tile_top + tile/2 - dshh/2 - dsh_bbox[1]),
           "dsh", font=dsh_font, fill=BRAND_DK)
...
def make_banner():
    ...
    glow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    gd = ImageDraw.Draw(glow)
    gd.ellipse([W*0.62, -H*0.4, W*1.1, H*1.0], fill=(77, 107, 254, 22))
    ...
    btile = 66 * S
    rounded_rect(d, [mx, 48*S, mx + btile, 48*S + btile], 14*S, BRAND)
    bdsh = black(38 * S)
    bb = d.textbbox((0, 0), "dsh", font=bdsh)
    bw, bh = bb[2]-bb[0], bb[3]-bb[1]
    d.text((mx + btile/2 - bw/2 - bb[0], 48*S + btile/2 - bh/2 - bb[1]), "dsh", font=bdsh, fill=BRAND_DK)
    ...
    d.text((mx + l2a_w, y2), line2b, font=f_slogan, fill=BRAND)
